...
export default function ChordProViewComponentAlt({
  ...
  const songKey = useMemo(() => {
    const baseKey = setListSong.key || setListSong.upload_key || "C";
    const baseIndex = keys.indexOf(baseKey);
    if (baseIndex === -1) return baseKey;
    return keys[(baseIndex + transpose + 120) % 12]; // +120 to avoid negative mod
  }, [transpose, setListSong.key, setListSong.upload_key]);
