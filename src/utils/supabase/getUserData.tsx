...
export default async function fbasicUserData() {
  ...
  if (user) {
    const { data, error } = (await supabase
      .from("profiles")
      .select(
        "id,email, name, lastname, avatar_url, role(role_name), church(id,church_name,logo,accepting_signups),phone",
      )
    ...
    userData = {
      ...
      accepting_signups: data?.church?.accepting_signups || false,
    };
